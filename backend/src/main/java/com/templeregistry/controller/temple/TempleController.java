...
public class TempleController {
        ...
        /*
         * â”€â”€ Temple Profile Staging Workflow (TA â†’ DC approval)
         * â”€â”€â”€â”€â”€â”€â”€â”€â”€
         */
        ...
        @Operation(summary = "Upload multiple temple photos")
        public ResponseEntity<ApiResponse<List<String>>> uploadPhotos(
                        @PathVariable Long id,
                        @RequestPart("files") List<MultipartFile> files) {

                List<String> urls = templeService.uploadPhotos(id, files);
                return ResponseEntity.ok(ApiResponse.success("Temple photos uploaded.", urls));
        }

        @DeleteMapping("/{templeId}/photos/{photoId}")
        @Operation(summary = "Delete temple photo")
        public ResponseEntity<ApiResponse<Void>> deletePhoto(
                        @PathVariable Long templeId,
                        @PathVariable Long photoId) {

                templeService.deletePhoto(templeId, photoId);
                return ResponseEntity.ok(ApiResponse.success("Photo deleted."));
        }

        @GetMapping("/{id}/photos")
        public ResponseEntity<ApiResponse<List<TemplePhotoDto>>> getPhotos(@PathVariable Long id) {
                List<TemplePhotoDto> photos = templeService.getTemplePhotos(id);
