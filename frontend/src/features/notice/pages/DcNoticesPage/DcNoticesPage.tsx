...
import { useDcNotices } from '../../noticeHooks'
import type { NoticeListFilter, NoticeStatus, NoticePriority } from '../../noticeTypes'
import { Bell } from 'lucide-react'

export function DcNoticesPage() {
  ...
  const { data, isLoading, page, goToPage, pageSize } = useDcNotices(filter)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Select
          value={status || 'all'}
          onValueChange={(v) => setStatus(v === 'all' ? '' : v as NoticeStatus)}
        >
        ...
        <Select
          value={priority || 'all'}
          onValueChange={(v) => setPriority(v === 'all' ? '' : v as NoticePriority)}
        >
          <SelectTrigger className="w-36">
      ...
      {!isLoading && notices.length === 0 && (
        <EmptyState
          ...
          description="Create your first notice using the button above."
          icon={<Bell size={40} />}
      ...
      {!isLoading && notices.length > 0 && (
        <div className="space-y-2">
          {notices.map((n) => (
            <NoticeListItem key={n.id} notice={n} canManage={n.scope === 'DISTRICT'} />
          ))}
        </div>
      )}
