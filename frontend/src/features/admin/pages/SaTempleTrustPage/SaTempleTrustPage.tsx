...
import {
  Building2, Users, Calendar, TrendingUp, Plus, Edit, Trash2, Eye, User, Phone, MapPin, Shield, CheckCircle2, AlertCircle, ChevronLeft, ChevronRight, Download, Loader2, ArrowLeft, Info,
} from 'lucide-react'
...
export function SaTempleTrustPage() {
  ...
  const memberForm = useForm<CreateBoardMemberRequest>({
    ...
    defaultValues: {
      fullName: '', aadhaarNumber: '', designation: '', appointmentDate: '',
      tenureEndDate: '', contactNumber: '', address: '',
    },
  })
  ...
  useEffect(() => {
    if (editingMemberId !== null) {
      ...
      if (memberToEdit) updateMemberForm.reset(mapBoardMemberToForm(memberToEdit))
    } else {
  ...
  const onSaveTrust = async (values: CreateTrustRequest | UpdateTrustRequest) => {
    try {
      ...
      if (trust) {
        ...
        const res = await updateTrust({ trustId: trust.id, body }).unwrap()
        trustId = res.data?.id ?? trust.id
        toast.success('Trust details updated')
      } else {
        const res = await createTrust({ templeId, body: values as CreateTrustRequest }).unwrap()
        trustId = res.data?.id
        toast.success('Trust registered successfully')
      }
      await submitTrust(trustId).unwrap()
      toast.success('Trust submitted for review.')
  ...
  const onCreateMeeting = async (values: CreateBoardMeetingRequest) => {
    if (!trust?.id) return
    if (meetingFile) {
      if (meetingFile.type !== 'application/pdf') { toast.error('Meeting minutes must be a PDF'); return }
      if (meetingFile.size > MAX_MINUTES_SIZE) { toast.error('Meeting minutes must be 10 MB or smaller'); return }
    }
    try {
  ...
  const handleMeetingDocument = async (meetingId: number, trustId: number, mode: 'preview' | 'download', meetingDate: string) => {
    ...
    try {
      ...
      if (!res.ok) { toast.error('Could not load meeting minutes. Please try again.'); return }
      const blob = await res.blob()
      const objectUrl = URL.createObjectURL(blob)
      if (mode === 'preview') {
        const newTab = window.open(objectUrl, '_blank')
        if (newTab) setTimeout(() => URL.revokeObjectURL(objectUrl), 60_000)
  ...
  const paginatedMembers = displayMembers.slice(memberPage * MEMBERS_PAGE_SIZE, (memberPage + 1) * MEMBERS_PAGE_SIZE)
  const viewingMember = viewingMemberId ? [...allCurrentMembers, ...allPastMembers].find(m => m.id === viewingMemberId) : null

  const backPath = ROUTE_PATHS.DC_TEMPLE_DETAIL.replace(':templeId', String(templeId)) + '?tab=trust'
  ...
  return (
    <div className="space-y-5 pb-10">
      ...
      <Alert className="border-blue-200 bg-blue-50 text-blue-800">
        ...
        <AlertDescription>
          You are editing as <strong>Super Administrator</strong>. Trust details will be auto-submitted through the workflow on save.
        </AlertDescription>
      </Alert>
        ...
        <CardContent className="space-y-4 p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">Manage trust details, board members, and financial records</p>
            </div>
          </div>
      ...
      {!trust && !showTrustForm ? (
        <EmptyState
          ...
          description="Register this temple trust before submitting board, meeting, and financial details."
        ...
        <Tabs value={tab} onValueChange={(value) => { setTab(value); setPage(0) }} className="w-full">
          <div className="rounded-lg border border-border/60 bg-card/95 p-1 shadow-sm">
            <TabsList className="grid w-full grid-cols-4 gap-1 bg-transparent p-0">
              <TabsTrigger value="details" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm">Trust Details</TabsTrigger>
              <TabsTrigger value="board" disabled={!trust} className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm">Board Members</TabsTrigger>
              <TabsTrigger value="meetings" disabled={!trust} className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm">Meetings</TabsTrigger>
              <TabsTrigger value="financials" disabled={!trust} className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm">Financials</TabsTrigger>
            </TabsList>
          </div>
                  ...
                  <Card className="border-border/60 bg-card/95 shadow-sm">
                    <CardContent className="p-5 space-y-4">
                        ...
                        <FormField control={trustForm.control} name="trustType" render={({ field }) => (
                          <FormItem><FormLabel>Trust Type *</FormLabel>
                            <Select onValueChange={field.onChange} value={field.value}>
                              ...
                              <SelectContent>{TRUST_TYPES.map((type) => <SelectItem key={type} value={type}>{type.replace(/_/g, ' ')}</SelectItem>)}</SelectContent>
                            </Select><FormMessage />
                          </FormItem>
                        )} />
                      ...
                      <div className="flex gap-3 pt-2">
                        <Button type="submit" className="bg-gradient-gold shadow-gold" disabled={creating || updating || submittingTrust}>
                          {(creating || updating || submittingTrust) ? 'Saving...' : trust ? 'Save & Submit' : 'Register & Submit'}
                        </Button>
                ...
                <CardContent className="p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">Workflow status</p>
                    </div>
              ...
              <Form {...memberForm}>
                <form onSubmit={memberForm.handleSubmit(onAddMember)} className="rounded-lg border border-border bg-card p-4 space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <FormField control={memberForm.control} name="fullName" render={({ field }) => (<FormItem><FormLabel>Full Name *</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={memberForm.control} name="aadhaarNumber" render={({ field }) => (<FormItem><FormLabel>Aadhaar *</FormLabel><FormControl><Input inputMode="numeric" {...field} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={memberForm.control} name="designation" render={({ field }) => (<FormItem><FormLabel>Designation *</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={memberForm.control} name="appointmentDate" render={({ field }) => (<FormItem><FormLabel>Appointment Date *</FormLabel><FormControl><Input type="date" {...field} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={memberForm.control} name="tenureEndDate" render={({ field }) => (<FormItem><FormLabel>Tenure End Date</FormLabel><FormControl><Input type="date" {...field} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={memberForm.control} name="contactNumber" render={({ field }) => (<FormItem><FormLabel>Contact Number *</FormLabel><FormControl><Input inputMode="tel" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  </div>
                  <FormField control={memberForm.control} name="address" render={({ field }) => (<FormItem><FormLabel>Address *</FormLabel><FormControl><Textarea rows={3} {...field} /></FormControl><FormMessage /></FormItem>)} />
            ...
            {editingMemberId != null && (
              <Form {...updateMemberForm}>
                <form onSubmit={updateMemberForm.handleSubmit(onUpdateMember)} className="rounded-lg border border-border bg-card p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <FormField control={updateMemberForm.control} name="fullName" render={({ field }) => (<FormItem><FormLabel>Full Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={updateMemberForm.control} name="designation" render={({ field }) => (<FormItem><FormLabel>Designation</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={updateMemberForm.control} name="contactNumber" render={({ field }) => (<FormItem><FormLabel>Contact Number</FormLabel><FormControl><Input inputMode="tel" {...field} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={updateMemberForm.control} name="tenureEndDate" render={({ field }) => (<FormItem><FormLabel>Tenure End Date</FormLabel><FormControl><Input type="date" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  </div>
                  <FormField control={updateMemberForm.control} name="address" render={({ field }) => (<FormItem><FormLabel>Address</FormLabel><FormControl><Textarea rows={2} {...field} /></FormControl><FormMessage /></FormItem>)} />
                  <div className="flex gap-2">
            ...
            {membersLoading ? <TableBodySkeleton rows={4} cols={5} /> : (
              <>
                <div className="inline-flex rounded-lg border border-border/60 bg-card/95 p-1 shadow-sm">
                  <button onClick={() => { setMemberTab('current'); setMemberPage(0) }} className={`rounded-md px-4 py-2 text-sm font-medium transition-colors ${memberTab === 'current' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                    Current Members ({allCurrentMembers.length})
                  </button>
                  <button onClick={() => { setMemberTab('past'); setMemberPage(0) }} className={`rounded-md px-4 py-2 text-sm font-medium transition-colors ${memberTab === 'past' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                    Past Members ({allPastMembers.length})
                  </button>
                </div>
                <div className="animate-in fade-in-50 duration-300" key={memberTab}>
                  <MemberTable members={paginatedMembers} onDelete={onDeleteMember} onEdit={(id) => setEditingMemberId(id)} deleting={deletingMember} onView={(id) => setViewingMemberId(id)} />
                </div>
                {totalMemberPages > 1 && (
                  <div className="flex items-center justify-between rounded-lg border border-border/60 bg-card/95 px-4 py-3 shadow-sm">
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" onClick={() => setMemberPage(p => Math.max(0, p - 1))} disabled={memberPage === 0}><ChevronLeft size={16} className="mr-1" />Previous</Button>
                      <Button variant="outline" size="sm" onClick={() => setMemberPage(p => Math.min(totalMemberPages - 1, p + 1))} disabled={memberPage >= totalMemberPages - 1}>Next<ChevronRight size={16} className="ml-1" /></Button>
                    </div>
                  </div>
                )}
              </>
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
              ...
              {viewingMember && (
                <div className="space-y-5">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex flex-col items-end gap-2 shrink-0 ml-auto">
                        {viewingMember.isCurrent
                          ? <span className="inline-flex items-center mr-5 gap-1 rounded-full bg-green-500/10 px-2.5 py-0.5 text-xs font-medium text-green-600 whitespace-nowrap"><CheckCircle2 size={12} />Current</span>
                          : <span className="inline-flex items-center gap-1 rounded-full bg-gray-500/10 px-2.5 py-0.5 text-xs font-medium text-gray-600 whitespace-nowrap">Past</span>
                        }
                      </div>
                    </div>
                  </div>
                  <div>
                    <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2"><User size={16} className="text-primary" />Personal Information</h4>
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2"><Calendar size={16} className="text-primary" />Tenure Information</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <ModalInfoCard icon={<Calendar size={16} />} label="Appointment Date" value={viewingMember.appointmentDate ? new Date(viewingMember.appointmentDate).toLocaleDateString('en-IN') : 'Not specified'} />
                      <ModalInfoCard icon={<Calendar size={16} />} label="Tenure End Date" value={viewingMember.tenureEndDate ? new Date(viewingMember.tenureEndDate).toLocaleDateString('en-IN') : 'Not specified'} />
                    </div>
                  </div>
                </div>
              )}
          ...
          <TabsContent value="meetings" className="mt-6 space-y-4 animate-in fade-in-50 duration-300">
            ...
            {showMeetingForm && (
              <Form {...meetingForm}>
                <form onSubmit={meetingForm.handleSubmit(onCreateMeeting)} className="rounded-lg border border-border bg-card p-4 space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <FormField control={meetingForm.control} name="meetingDate" render={({ field }) => (<FormItem><FormLabel>Meeting Date *</FormLabel><FormControl><Input type="date" {...field} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={meetingForm.control} name="agenda" render={({ field }) => (<FormItem><FormLabel>Agenda</FormLabel><FormControl><Input placeholder="Agenda summary" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  </div>
            ...
            {meetingsLoading ? <TableBodySkeleton rows={3} cols={2} /> : meetings.length === 0 ? (
                    ...
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      ...
                      <div className="flex items-center gap-2 shrink-0 flex-wrap">
                        {meeting.minutesDocumentId ? (
                          <>
                            <span className="inline-flex items-center gap-1 text-xs text-emerald-600 font-medium"><CheckCircle2 size={12} />Minutes uploaded</span>
                            <Button variant="outline" size="sm" className="h-7 px-2.5 text-xs gap-1" disabled={!!docLoading[`${meeting.id}-preview`]} onClick={() => handleMeetingDocument(meeting.id, trust!.id, 'preview', meeting.meetingDate)}>
                              {docLoading[`${meeting.id}-preview`] ? <Loader2 size={13} className="animate-spin" /> : <Eye size={13} />}Preview
                            </Button>
                            <Button variant="outline" size="sm" className="h-7 px-2.5 text-xs gap-1" disabled={!!docLoading[`${meeting.id}-download`]} onClick={() => handleMeetingDocument(meeting.id, trust!.id, 'download', meeting.meetingDate)}>
                              {docLoading[`${meeting.id}-download`] ? <Loader2 size={13} className="animate-spin" /> : <Download size={13} />}Download
                            </Button>
                          </>
                        ) : <span className="inline-flex items-center gap-1 text-xs text-muted-foreground"><AlertCircle size={12} />No minutes</span>}
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <TabsContent value="financials" className="mt-6 space-y-4 animate-in fade-in-50 duration-300">
                ...
                <form onSubmit={financialForm.handleSubmit(onSubmitFinancial)} className="rounded-lg border border-border bg-card p-4 space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <FormField control={financialForm.control} name="financialYear" render={({ field }) => (
                      <FormItem><FormLabel>Financial Year *</FormLabel>
                        <Select onValueChange={field.onChange} value={field.value || ''}>
                          <FormControl><SelectTrigger><SelectValue placeholder="Select financial year" /></SelectTrigger></FormControl>
                          <SelectContent>{financialYearOptions.map((year) => <SelectItem key={year} value={year}>{year}</SelectItem>)}</SelectContent>
                        </Select><FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={financialForm.control} name="annualIncome" render={({ field }) => (<FormItem><FormLabel>Annual Income</FormLabel><FormControl><Input type="number" min={0} step="0.01" value={field.value ?? ''} onChange={(e) => field.onChange(e.target.value === '' ? null : Number(e.target.value))} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={financialForm.control} name="annualExpenditure" render={({ field }) => (<FormItem><FormLabel>Annual Expenditure</FormLabel><FormControl><Input type="number" min={0} step="0.01" value={field.value ?? ''} onChange={(e) => field.onChange(e.target.value === '' ? null : Number(e.target.value))} /></FormControl><FormMessage /></FormItem>)} />
                  </div>
                  <div className="flex gap-2">
            ...
            {financialsLoading ? <TableBodySkeleton rows={3} cols={4} /> : financials.length === 0 ? (
...
function MemberTable({ members, onDelete, onEdit, onView, deleting }: {
  members: Array<{ id: number; fullName: string; designation?: string; appointmentDate?: string; contactNumber?: string; maskedAadhaar?: string | null; address?: string; isCurrent: boolean }>
              ...
              {members.map((member) => (
                <tr key={member.id} className="hover:bg-muted/10 transition-colors">
                  <td className="px-4 py-3">
                    ...
                    {member.maskedAadhaar && <div className="text-xs text-muted-foreground mt-0.5">{member.maskedAadhaar}</div>}
                  </td>
                  <td className="px-4 py-3 text-muted-foreground">{member.designation ?? '—'}</td>
                  <td className="px-4 py-3 text-muted-foreground">{member.appointmentDate ? new Date(member.appointmentDate).toLocaleDateString('en-IN') : '—'}</td>
                  <td className="px-4 py-3 text-muted-foreground">{member.contactNumber ?? '—'}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-1">
                      <Button variant="ghost" size="sm" onClick={() => onView(member.id)} className="h-8 w-8 p-0" title="View details"><Eye size={14} /></Button>
                      <Button variant="ghost" size="sm" onClick={() => onEdit(member.id)} className="h-8 w-8 p-0" title="Edit member"><Edit size={14} /></Button>
                      <Button variant="ghost" size="sm" disabled={deleting} onClick={() => void onDelete(member.id)} className="h-8 w-8 p-0 text-destructive hover:text-destructive" title="Delete member"><Trash2 size={14} /></Button>
                    </div>
                  </td>
                </tr>
              ))}
...
function ModalInfoCard({ icon, label, value, className = '' }: { icon: React.ReactNode; label: string; value: string; className?: string }) {
  return (
